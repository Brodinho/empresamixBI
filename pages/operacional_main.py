...
from modules.operacional.views.visualizations import (
    producao,
    qualidade,
    estoque,
    manutencao
...
def load_operacional_module():
    logger.debug("=== INICIANDO MÓDULO OPERACIONAL ===")
    
    # Configuração DEVE ser a primeira chamada Streamlit
    st.set_page_config(
        page_title="Módulo Operacional",
        layout="wide",
    ...
    menu_items = {
        "Produção": producao.render,
        "Controle de Qualidade": qualidade.render,
        "Gestão de Estoque": estoque.render,
        "Manutenção": manutencao.render
    }
...
if __name__ == "__main__":
    try:
        load_operacional_module()
    except Exception as e:
        logger.error(f"Erro ao carregar módulo operacional: {str(e)}") 